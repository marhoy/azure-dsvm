...
#c.LabApp.app_dir = '/data/miniconda/envs/jupyterhub/share/jupyter/lab'
...
#c.LabApp.user_settings_dir = '/data/home/hoyma/.jupyter/lab/user-settings'

## Whether to serve the app in watch mode
#c.LabApp.watch = False

## The directory for workspaces
#c.LabApp.workspaces_dir = '/data/home/hoyma/.jupyter/lab/workspaces'

#------------------------------------------------------------------------------
# SingleUserNotebookApp(NotebookApp) configuration
#------------------------------------------------------------------------------

## A Subclass of the regular NotebookApp that is aware of the parent multiuser
#  context.

## Disable user configuration of single-user server.
#  
...
#c.SingleUserNotebookApp.disable_user_config = False

## 
#c.SingleUserNotebookApp.group = ''

## 
#c.SingleUserNotebookApp.hub_api_url = ''

## 
#c.SingleUserNotebookApp.hub_host = ''

## 
#c.SingleUserNotebookApp.hub_prefix = '/hub/'

## 
#c.SingleUserNotebookApp.user = ''

#------------------------------------------------------------------------------
# SingleUserLabApp(SingleUserNotebookApp,LabApp) configuration
#------------------------------------------------------------------------------

## A sublcass of JupyterHub's SingleUserNotebookApp which includes LabApp as a
#  mixin. This makes the LabApp configurables available to the spawned jupyter
#  server.
#  
#  If you don't need to change any of the configurables from their default
#  values, then this class is not necessary, and you can deploy JupyterLab by
#  ensuring that its server extension is enabled and setting the
#  `Spawner.default_url` to '/lab'.
#  
#  If you do need to configure JupyterLab, then use this application by setting
...
#------------------------------------------------------------------------------
# HubAuth(SingletonConfigurable) configuration
#------------------------------------------------------------------------------
...
#  
#  If using tornado, use via :class:`HubAuthenticated` mixin. If using manually,
#  use the ``.user_for_cookie(cookie_value)`` method to identify the user
#  corresponding to a given cookie value.
#  
...
#  
#  Generate with `jupyterhub token [username]` or add to JupyterHub.services
#  config.
#c.HubAuth.api_token = ''
...
#c.HubAuth.api_url = 'http://127.0.0.1:8081/hub/api'
...
#  
#  A larger value reduces load on the Hub and occasional response lag. A smaller
#  value reduces propagation time of changes on the Hub (rare).
#  
#  Default: 300 (five minutes)
#c.HubAuth.cache_max_age = 300

## The name of the cookie I should be looking for
#c.HubAuth.cookie_name = 'jupyterhub-services'

## Additional options to pass when setting cookies.
#  
#  Can include things like `expires_days=None` for session-expiry or
#  `secure=True` if served on HTTPS and default HTTPS discovery fails (e.g.
#  behind some proxies).
#c.HubAuth.cookie_options = {}
...
#  
#  Typically /hub/
#c.HubAuth.hub_prefix = '/hub/'

## The login URL to use
#  
#  Typically /hub/login
#c.HubAuth.login_url = '/hub/login'

#------------------------------------------------------------------------------
# HubOAuth(HubAuth) configuration
#------------------------------------------------------------------------------
